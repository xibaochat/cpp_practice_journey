...
void OfficeBlock::doBureaucracy(std::string form, std::string target)
{
	...
	if (!(ptr_f = this->m_intern->makeForm(form, target)))
		return ;
	std::cout << YELLOW << "In OfficeBlock::doBureaucracy " << NC << "Form: " << form << " Required sign grade " << ptr_f->get_signed_grade() << " Required executor grade " << ptr_f->get_exe_grade() << std::endl;
	try
	{
